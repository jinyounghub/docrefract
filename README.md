...
> [!IMPORTANT]
> DocRefract is an early alpha. Pin version `0.2.0` in automation and review the [current boundaries](#current-boundaries) before using it as a release gate.
...
```console
dotnet tool install --global DocRefract.Tool --version 0.2.0
```
...
```console
dotnet tool update --global DocRefract.Tool --version 0.2.0
```
...
If the global tool is unsuitable, the [v0.2.0 GitHub Release](https://github.com/jinyounghub/docrefract/releases/tag/v0.2.0) provides six self-contained fallback archives that do not require a separately installed .NET runtime.

| Platform | Architecture | Release asset |
| --- | --- | --- |
| Linux | x64 | `docrefract-0.2.0-linux-x64.tar.gz` |
| Linux | Arm64 | `docrefract-0.2.0-linux-arm64.tar.gz` |
| macOS | Intel x64 | `docrefract-0.2.0-osx-x64.tar.gz` |
| macOS | Apple silicon | `docrefract-0.2.0-osx-arm64.tar.gz` |
| Windows | x64 | `docrefract-0.2.0-win-x64.zip` |
| Windows | Arm64 | `docrefract-0.2.0-win-arm64.zip` |

For example, on Linux x64:

```console
tar -xzf docrefract-0.2.0-linux-x64.tar.gz
./docrefract-0.2.0-linux-x64/docrefract --version
```

On Windows x64 in PowerShell:

```powershell
Expand-Archive .\docrefract-0.2.0-win-x64.zip
.\docrefract-0.2.0-win-x64\docrefract.exe --version
```
...
```console
gh release download v0.2.0 --repo jinyounghub/docrefract --dir .docrefract-release
  ...
  compare:
    ...
    steps:
      ...
      - name: Compare generated document
        uses: jinyounghub/docrefract@v0.2.0
        with:
...
v0.2.0 intentionally does not include:
...
dotnet tool install --global DocRefract.Tool --version 0.2.0 --source artifacts/packages
```
